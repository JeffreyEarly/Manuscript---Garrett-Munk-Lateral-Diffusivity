...
w_lin = wavemodel.VariableFieldsAtTime(t,'w');
dw = w-w_lin;

w_bar = SineTransformForward(wavemodel.z,FourierTransformForward(wavemodel.y,FourierTransformForward(wavemodel.x,w,1),2));
w_bar = cat(3,w_bar,zeros(wavemodel.Nx,wavemodel.Ny));

w_lin_bar = SineTransformForward(wavemodel.z,FourierTransformForward(wavemodel.y,FourierTransformForward(wavemodel.x,w_lin,1),2));
w_lin_bar = cat(3,w_lin_bar,zeros(wavemodel.Nx,wavemodel.Ny));

dw_bar = w_bar-w_lin_bar;
...
dwVariance = zeros(nK,nModes);
wVariance = zeros(nK,nModes);
wVariancePredicted = zeros(nK,nModes);
...
for iMode = 1:1:nModes
    for iK = 1:1:nK
        ...
        for iIndex = 1:length(indicesForK)
            ...
            dwVariance(iK,iMode) = dwVariance(iK,iMode) + dw_bar(i,m,iMode)^2;
            wVariance(iK,iMode) = wVariance(iK,iMode) + w_bar(i,m,iMode)^2;
            wVariancePredicted(iK,iMode) = wVariancePredicted(iK,iMode) + w_lin_bar(i,m,iMode)^2;
        end
    end
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
figure
jpcolor( k, j, log10(dwVariance./wVariance).' );  xlog, ylog, shading flat, caxis([-10 0])

FigureSize = [50 50 figure_width_2col+8 225*scaleFactor];
fig1 = figure('Units', 'points', 'Position', FigureSize,'Name','Vertical velocity variance');
set(gcf, 'Color', 'w');
...
p1 = subplot(1,2,1);
jpcolor( k, j, log10(wVariance.') ); xlog, ylog, shading flat, hold on
...
title('w variance (m^2/s^2)', 'FontSize', figure_axis_label_size, 'FontName', figure_font);
...
p2 = subplot(1,2,2);
jpcolor( k, j, log10(wVariancePredicted.') ); xlog, ylog, shading flat, hold on
plot( k_damp, j_damp, 'LineWidth', 4, 'Color', 0*[1 1 1])
plot( k_damp, j_damp, 'LineWidth', 2, 'Color', [1 1 1])
...
fig1 = figure('Units', 'points', 'Position', FigureSize,'Name','Linear/Actual');
set(gcf, 'Color', 'w');
fig1.PaperUnits = 'points';
fig1.PaperPosition = FigureSize;
fig1.PaperSize = [FigureSize(3) FigureSize(4)];
HKE_fraction = wVariancePredicted./wVariance;
...
jpcolor( k, j, (log10(HKE_fraction.') )); xlog, ylog, shading flat, hold on
plot( k_damp, j_damp, 'LineWidth', 4, 'Color', 0*[1 1 1])
plot( k_damp, j_damp, 'LineWidth', 2, 'Color', [1 1 1])
...
caxis(log10([1/1.1 1.1]))
cb3.Ticks = (log10([(0.9) (0.99) (1.0) (1.01) (1.10)]));
cb3.TickLabels = {'-10%', '-1%', '0%', '1%', '10%'};
